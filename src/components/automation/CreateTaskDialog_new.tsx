...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({ onCreateTask, isLoading = false }) => {
  ...
  const taskTemplates = [
    ...
    },
    {
      ...
      event: { scheduleType: 'recurring' as const, frequency: 'weekly' as const, time: '18:00' }
    },
    {
    ...
    },
    {
      ...
      event: { scheduleType: 'recurring' as const, frequency: 'monthly' as const, time: '08:00' }
    }
  ];
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Select 
                        ...
                        onValueChange={(value: 'daily' | 'weekly' | 'monthly') => updateEventConfig({ frequency: value })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                  ...
                  <CardContent>
                    <div className="text-sm text-muted-foreground">
                      {formData.event.scheduleType === 'one-time' ? (
                        ...
                        `Will run ${formData.event.frequency || 'daily'} at ${formData.event.time || '09:00'}`
                      )}
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => useTemplate(template)}>
                      ...
                      <CardContent>
                        ...
                        <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
                          ...
                          <span>
                            {template.event.frequency} at {template.event.time}
                          </span>
                        </div>
                      </CardContent>
                    </Card>
